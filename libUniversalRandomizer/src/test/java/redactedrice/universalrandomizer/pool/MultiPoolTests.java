...
class MultiPoolTests {
    ...
    void reset() {
        ...
        assertEquals(NON_DUPLICATE_VALS.size() - 2, p1.size(),
        ...
        assertEquals(NON_DUPLICATE_VALS.size() - 2, p1.size(),
                "size returned wrong size for item pool");
        assertEquals(DUPLICATE_VALS.size() - 1, p2.size(),
                "size returned wrong size for item pool");

        mp.reset();
        assertEquals(NON_DUPLICATE_VALS.size(), p1.size(),
                "size returned wrong size for item pool after reset");
        assertEquals(DUPLICATE_VALS.size(), p2.size(),
                "size returned wrong size for item pool after reset");
    }
    ...
    void useNextPool() {
        ...
        mp.get(rand);
        assertTrue(mp.useNextPool());
        assertEquals(NON_DUPLICATE_VALS.size() - 1, p1.getWorkingPools().get(0).size(),
                "size did not return the full pool size");
        assertEquals(NON_DUPLICATE_VALS.size(), p1.getWorkingPools().get(1).size(),
                "size did not return the full pool size");
